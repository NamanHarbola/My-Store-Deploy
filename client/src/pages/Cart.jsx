  ...
  const Cart = () => {
    ...
    // Calculate subtotal, tax and totalAmount on cartArray change
    useEffect(() => {
      // Calculate subtotal from cartArray items
      const subtotal = cartArray.reduce(
        (acc, item) => acc + item.offerPrice * item.quantity,
        0
      );

      // Calculate tax at 2.11%
      const tax = +(subtotal * 0.0211).toFixed(2);
